...
def parse_args():
    ...
    parser.add_argument(
        "--scheduler-address", "-a", type=str, required=True, help="Scheduler address (e.g., tcp://localhost:2345)"
    )
    parser.add_argument(
        "--object-storage-address", type=str, default=None, help="Object storage address (e.g., localhost:2346)"
    )

    # AWS Batch configuration
    parser.add_argument("--job-queue", "-q", type=str, help="AWS Batch job queue name (required for batch backend)")
    parser.add_argument(
        "--job-definition", "-d", type=str, help="AWS Batch job definition name (required for batch backend)"
    )
    parser.add_argument("--aws-region", type=str, default="us-east-1", help="AWS region")
    parser.add_argument("--s3-bucket", type=str, help="S3 bucket for task data (required for batch backend)")
    parser.add_argument("--s3-prefix", type=str, default="scaler-tasks", help="S3 prefix for task data")

    # Worker configuration
    parser.add_argument("--max-concurrent-jobs", type=int, default=100, help="Maximum concurrent jobs")
    parser.add_argument("--job-timeout", type=int, default=60, help="Job timeout in minutes (default: 60 = 1 hour)")
    parser.add_argument("--heartbeat-interval", type=int, default=1, help="Heartbeat interval in seconds")
    parser.add_argument("--death-timeout", type=int, default=30, help="Death timeout in seconds")
    parser.add_argument("--name", type=str, default=None, help="Worker name (default: aws-<backend>-worker)")
