...
from scaler.worker_adapter.orb.helper import ORBException, ORBHelper
...
class ORBAdapter:
    ...
    def _create_user_data(self) -> str:
        ...
        # Build the command
        cmd_parts = [
            f"/usr/local/bin/scaler_cluster {adapter_config.scheduler_address.to_address()}",
            f"--num-of-workers {num_workers}",
            f'--worker-names "ORB|$(curl -s http://169.254.169.254/latest/meta-data/instance-id)"',
            ...
            f"--event-loop {self._config.event_loop}",
            f"--worker-io-threads {self._config.worker_io_threads}",
        ]

        if worker_config.hard_processor_suspend:
            cmd_parts.append("--hard-processor-suspend")

        if adapter_config.object_storage_address:
            cmd_parts.append(f"--object-storage-address {adapter_config.object_storage_address.to_string()}")

        capabilities = worker_config.per_worker_capabilities.capabilities
        if capabilities:
            cap_str = format_capabilities(capabilities)
            if cap_str.strip():
                cmd_parts.append(f"--per-worker-capabilities {cap_str}")

        command = " ".join(cmd_parts)

        script = f"""#!/bin/bash\nnohup {command} > /var/log/opengris-scaler.log 2>&1 &\n"""
    ...
    def _create_security_group(self):
        ...
        sg_response = self._ec2.create_security_group(
            Description="Security group for OpenGRIS ORB worker adapter",
    ...
    async def start_worker_group(self) -> WorkerGroupID:
        ...
        worker_name = f"ORB|{instance_id}"
        worker_id = WorkerID.generate_worker_id(worker_name)
