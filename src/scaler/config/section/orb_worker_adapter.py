...
class ORBWorkerAdapterConfig(ConfigClass):
    ...
    orb_config_path: str = dataclasses.field(
        default="orb_config.json", metadata=dict(help="Path to the ORB configuration file")
    )

    instance_type: str = dataclasses.field(default="t2.micro", metadata=dict(help="EC2 instance type"))
    aws_region: Optional[str] = dataclasses.field(default=None, metadata=dict(help="AWS region (optional, defaults to environment/profile settings)"))
    ...
    allowed_ip: str = dataclasses.field(
        default="",
        metadata=dict(help="IP address to allow in the security group (if created automatically). Defaults to current public IP."),
    )
    ...
    def __post_init__(self) -> None:
        ...
        if not self.orb_config_path:
            raise ValueError("orb_config_path must be provided.")
