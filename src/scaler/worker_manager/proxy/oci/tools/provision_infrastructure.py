...
Simple provisioning for OCI resources required by the Scaler OCI proxy worker manager:
...
class OCIInfrastructureProvisioner:
    """
        ...
        try:
            response = self._identity.create_dynamic_group(
                create_dynamic_group_details=oci.identity.models.CreateDynamicGroupDetails(
                    ...
                    description=f"Scaler OCI worker manager, compartment {self._compartment_id}",
                )
            )
        ...
        try:
            response = self._identity.create_policy(
                create_policy_details=oci.identity.models.CreatePolicyDetails(
                    ...
                    description="Scaler OCI worker manager - Object Storage access for container instances",
                )
            )
    ...
    def build_and_push_image(self) -> str:
        ...
        dockerfile_path = Path(__file__).parent.parent / "remote" / "Dockerfile"
        # The Dockerfile COPYs paths of the form scaler/..., so the build context is the src/ directory.
        # This file is src/scaler/worker_manager/proxy/oci/tools/provision_infrastructure.py, so
        # parents[5] is src/ (tools, oci, proxy, worker_manager, scaler, src).
        src_root = Path(__file__).parents[5]
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Provision OCI resources for Scaler OCI proxy worker manager")
    parser.add_argument(
    ...
    provisioner = OCIInfrastructureProvisioner(
    ...
    if args.action == "show":
        try:
            config = OCIInfrastructureProvisioner.load_config(args.config)
            print("\n=== Saved OCI Config ===")
    ...
    if args.action == "provision":
        ...
        OCIInfrastructureProvisioner.save_config(result, args.config)
        OCIInfrastructureProvisioner.save_env_file(result, args.env_file)
