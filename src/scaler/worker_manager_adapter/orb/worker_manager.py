...
class ORBWorkerAdapter:
    ...
    def _create_user_data(self) -> str:
        ...
        # NOTE: --num-of-workers is not passed; NativeWorkerManager defaults to cpu_count - 1
        # workers in fixed mode, where cpu_count is determined by the machine type configured by the user.
        script = f"""#!/bin/bash
nohup /usr/local/bin/scaler_worker_manager native {adapter_config.scheduler_address.to_address()} \
    --mode fixed \
    --worker-type ORB \
