...
TcpClient::TcpClient(
    ...
    , _eventManager(std::make_shared<EventManager>(
          _eventLoopThread, std::move(*FileDescriptor::socket(0, 0, 0)), [](FileDescriptor& fd, Events events) {}))
    , _connected(false) {
...
void TcpClient::onWrite() {
    ...
    // auto& sock     = this->_eventLoopThread->_identityToIOSocket.at(id);
    // FIXME: the second _addr is not real
    // sock->_fdToConnection[_connFd] = std::make_unique<MessageConnectionTCP>(_eventLoopThread, _connFd, addr, addr,
    // id); sock->_fdToConnection[_connFd]->onCreated();
    _connected = true;
}
