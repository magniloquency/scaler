...
void IOSocket::onCreated() {
    ...
    if (_socketType == IOSocketType::Dealer) {
        ...
        _tcpServer = new TcpServer(shared_from_this());
...
void IOSocket::sendMessage(Message message, Configuration::SendMessageCallback callback) {
    _eventLoopThread->_eventLoop->executeNow(
        // [this, message = std::move(message), callback = std::move(callback)] mutable {
        //     // TODO: What should we do when we cannot find the connection? We cannot
        //     // check whether the identity presents outside the eventloop.
        //     auto addr = message.address.as_string();
        //     auto* conn = this->_identityToConnection.at(addr);

        //     // todo: why does this not compile?
        //     // conn->sendMessage(std::move(message.payload), std::move(callback));
        // }
        []() {});
}

void IOSocket::recvMessage(Configuration::RecvMessageCallback callback) {
    _eventLoopThread->_eventLoop->executeNow([this, callback = std::move(callback)] mutable {
        // TODO: recv is a socket-level operation and receives from all connections
    });
}
