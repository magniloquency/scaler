...
int main() {
    ...
    clientSocket->connectTo("tcp://127.0.0.1:8080", [connect_promise](auto result) { connect_promise->set_value(); });
    ...
    while (cnt++ < 10) {
        ...
        clientSocket->sendMessage(std::move(message), [send_promise](auto) { send_promise->set_value(); });
        ...
        clientSocket->recvMessage([recv_promise](auto msg) { recv_promise->set_value(std::move(*msg)); });
