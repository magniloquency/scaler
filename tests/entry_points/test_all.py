...
class TestScalerAllConfigShape(unittest.TestCase):
    ...
    def test_orb_worker_manager_parsed_from_toml(self) -> None:
        ...
        config = self._parse(toml)
        self.assertEqual(len(config.worker_managers), 1)
        self.assertIsInstance(config.worker_managers[0], ORBWorkerAdapterConfig)

    def test_orb_fields_from_toml(self) -> None:
        toml = {
            "worker_manager": {
                "type": "orb",
                "scheduler_address": "tcp://127.0.0.1:6378",
    ...
    def test_mixed_native_and_orb_worker_managers(self) -> None:
        from scaler.config.section.native_worker_manager import NativeWorkerManagerConfig
        from scaler.config.section.orb_worker_adapter import ORBWorkerAdapterConfig
        ...
        toml = {
            "worker_manager": [
                {"type": "baremetal_native", "scheduler_address": "tcp://127.0.0.1:6378", "worker_manager_id": "wm-1"},
                {"type": "orb", "scheduler_address": "tcp://127.0.0.1:6378", "image_id": "ami-0528819f94f4f5fa5"},
            ]
        }
        config = self._parse(toml)
        self.assertEqual(len(config.worker_managers), 2)
        self.assertIsInstance(config.worker_managers[0], NativeWorkerManagerConfig)
        self.assertIsInstance(config.worker_managers[1], ORBWorkerAdapterConfig)
...
class TestRunWorkerManager(unittest.TestCase):
    ...
    def _make_orb_config(self, event_loop="builtin", logging_level="INFO"):
        from scaler.config.common.logging import LoggingConfig
        from scaler.config.common.worker import WorkerConfig
        from scaler.config.common.worker_manager import WorkerManagerConfig
        from scaler.config.section.orb_worker_adapter import ORBWorkerAdapterConfig
    ...
    def test_orb_run_worker_manager_dispatches_correctly(self) -> None:
        from scaler.entry_points.scaler import _run_worker_manager

        config = self._make_orb_config()

        with patch("scaler.entry_points.scaler.setup_logger"), patch(
            "scaler.entry_points.scaler.register_event_loop"
        ), patch("scaler.worker_manager_adapter.orb.worker_manager.ORBWorkerAdapter") as mock_orb:
    ...
    def test_orb_register_event_loop_called(self) -> None:
        from scaler.entry_points.scaler import _run_worker_manager

        config = self._make_orb_config(event_loop="builtin")
