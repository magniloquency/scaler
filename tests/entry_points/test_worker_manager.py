...
_ORB_BASE_ARGV = ["tcp://127.0.0.1:6378", "--image-id", "ami-0528819f94f4f5fa5"]


class TestORBWorkerManagerSubcommand(unittest.TestCase):
    """Tests that ORBWorkerAdapterConfig is correctly parsed via parse_with_section."""

    def test_orb_image_id_parsed(self) -> None:
        from scaler.config.section.orb_worker_adapter import ORBWorkerAdapterConfig

        config = ORBWorkerAdapterConfig.parse_with_section("scaler_worker_manager", {}, argv=_ORB_BASE_ARGV)
        self.assertIsInstance(config, ORBWorkerAdapterConfig)
    ...
    def test_orb_instance_type_and_region_from_cli(self) -> None:
        ...
        config = ORBWorkerAdapterConfig.parse_with_section(
            "scaler_worker_manager",
            {},
            argv=[*_ORB_BASE_ARGV, "--instance-type", "t3.medium", "--aws-region", "eu-west-1"],
        )
    ...
    def test_orb_logging_level_from_cli(self) -> None:
        ...
        config = ORBWorkerAdapterConfig.parse_with_section(
            "scaler_worker_manager", {}, argv=[*_ORB_BASE_ARGV, "--logging-level", "DEBUG"]
        )
        self.assertEqual(config.logging_config.level, "DEBUG")
